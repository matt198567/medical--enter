(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".documentation__slider")) new core(".documentation__slider", {
            modules: [ Navigation, Pagination, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
    ...
    const script_images = document.querySelectorAll(".services__image-ibg");
    function handleHover(event) {
        event.preventDefault();
        const index = Array.from(links).indexOf(event.target);
        if (index >= 0 && index < script_images.length) {
            script_images.forEach((image => {
                image.classList.remove("active");
                image.style.display = "none";
            }));
            script_images[index].classList.add("active");
